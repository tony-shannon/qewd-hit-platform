...
OR
docker run -it --name openehr_service --rm --network qewd-hit -v ~/qewd-hit-platform/openehr-ms:/opt/qewd/mapped -v ~/hit-templates/clinical-headings-templates:/opt/qewd/mapped/templates  -e mode="microservice" rtweed/qewd-server
...
OR
docker run -it --rm --name ethercis-db --net ecis-net -p 5432:5432 -v ~/backup-stuff/ecis:/var/lib/postgresql/pgdata rtweed/ethercis-db 
